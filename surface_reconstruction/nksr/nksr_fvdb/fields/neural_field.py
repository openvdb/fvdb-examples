...
from nksr_fvdb.fields.base_field import BaseField, EvaluationResult
from nksr_fvdb.nn.encdec import MultiscalePointDecoder
from nksr_fvdb.svh import SparseFeatureHierarchy


class NeuralField(BaseField):
    def __init__(
        self, svh: SparseFeatureHierarchy, decoder: MultiscalePointDecoder, features: dict, grad_type: str = "numerical"
    ):
        super().__init__(svh)
            ...
            if self.grad_type == "numerical":
                ...
                with torch.enable_grad():
                    ...
                    grad_value = torch.autograd.grad(
                        res_d, [xyz_d], grad_outputs=torch.ones_like(res_d), create_graph=self.decoder.training
                    )[0]
        else:
            grad_value = None
