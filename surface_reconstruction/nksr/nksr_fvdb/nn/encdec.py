...
class ResnetBlockFC(nn.Module):
    ...
    def __init__(self, size_in: int, size_out: int | None = None):
        super().__init__()
        # Attributes
        if size_out is None:
            size_out = size_in
...
class PointEncoder(nn.Module):
    ...
    def __init__(self, dim: int, c_dim: int = 32, hidden_dim: int = 32, n_blocks: int = 3):
        super().__init__()

        self.c_dim = c_dim
        self.fc_pos = nn.Linear(dim, 2 * hidden_dim)
        self.blocks = nn.ModuleList([ResnetBlockFC(2 * hidden_dim, hidden_dim) for _ in range(n_blocks)])
        self.fc_c = nn.Linear(hidden_dim, c_dim)
        self.hidden_dim = hidden_dim

    def forward(self, pts_xyz: JaggedTensor, pts_feature: JaggedTensor, svh: SparseFeatureHierarchy, depth: int = 0):
        ...
        for block in self.blocks[1:]:
            pooled, _ = scatter_max(pts_feature, vid, dim=0, dim_size=grid.total_voxels)
            pooled = pooled[vid]
...
class MultiscalePointDecoder(nn.Module):
    ...
    def forward(self, xyz: JaggedTensor, svh: SparseFeatureHierarchy, multiscale_feat: dict[int, JaggedTensor]):
        p_feats = []
