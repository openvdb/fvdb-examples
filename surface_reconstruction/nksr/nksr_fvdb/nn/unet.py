...
from nksr_fvdb.svh import SparseFeatureHierarchy, VoxelStatus
    ...
    def __init__(self, in_channels: int, out_channels: int, order: str, num_groups: int, kernel_size: int = 3):
        ...
        for i, char in enumerate(order):
            if char == "r":
                ...
                self.add_module(
                    "Conv",
                    fvnn.SparseConv3d(in_channels, out_channels, kernel_size, 1, bias="g" not in order),
                )
            elif char == "g":
                if i < order.index("c"):
...
class SparseDoubleConv(nn.Sequential):
    def __init__(self, in_channels: int, out_channels: int, order: str, num_groups: int, encoder: bool):
        ...
        self.add_module("SingleConv1", SparseConvBlock(conv1_in_channels, conv1_out_channels, order, num_groups))
        self.add_module("SingleConv2", SparseConvBlock(conv2_in_channels, conv2_out_channels, order, num_groups))


class SparseHead(nn.Sequential):
    def __init__(self, in_channels: int, out_channels: int, order: str, num_groups: int, enhanced: bool = False):
        super().__init__()
        ...
        if enhanced:
            ...
            self.add_module("OneConv0", SparseConvBlock(mid_channels, mid_channels, order, num_groups, kernel_size=1))
...
class SparseStructureNet(nn.Module):
    def __init__(
        ...
        for layer_idx in range(num_blocks):
            self.encoders.add_module(
                f"Enc{layer_idx}",
                SparseDoubleConv(n_features[layer_idx], n_features[layer_idx + 1], order, num_groups, True),
            )
        ...
        for layer_idx in range(-1, -num_blocks - 1, -1):
            self.struct_heads.add_module(f"Struct{layer_idx}", SparseHead(n_features[layer_idx], 3, order, num_groups))

            if self.udf_branch_dim > 0:
                self.udf_heads.add_module(
                    f"UDF{layer_idx}", SparseHead(n_features[layer_idx], self.udf_branch_dim, order, num_groups)
                )
            if self.normal_channels == 0:
                self.normal_heads.add_module(f"Normal{layer_idx}", None)
            else:
                self.normal_heads.add_module(
                    f"Normal{layer_idx}", SparseHead(n_features[layer_idx], self.normal_channels, order, num_groups)
                )
            self.basis_heads.add_module(
                f"Basis{layer_idx}",
                SparseHead(n_features[layer_idx], self.basis_channels, order, num_groups, enhanced=True),
            )
    ...
    def build_neck_grid(self, sparse_grid: GridBatch, dense_svh: SparseFeatureHierarchy):
        ...
        if self.neck_type == "dense":
            ...
            dense_svh.build_from_grid_coords(
                dense_svh.depth - 1,
                sparse_coords,
    ...
    def forward(
        ...
        decoder_svh = SparseFeatureHierarchy(encoder_svh.voxel_size, encoder_svh.depth, encoder_svh.device)
        decoder_tmp_svh = SparseFeatureHierarchy(encoder_svh.voxel_size, encoder_svh.depth, encoder_svh.device)
        ...
        # Up-sample
        upsample_mask = None
        for module, upsampler, struct_conv, normal_conv, basis_conv, udf_conv in zip(
            ...
            upsample_mask = (
                decoder_svh.grids[feat_depth]
                .fill_to_grid(
                    (struct_decision == VoxelStatus.VS_EXIST_CONTINUE.value).float(), decoder_tmp_svh.grids[feat_depth]
                )
                .type(torch.bool)
            )
